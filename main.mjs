...
class UI {
  ...
  static addExpense(expense) {
    ...
    row.innerHTML = `
        ...
        <td class="exp-amount">₹${parseFloat(expense.amount).toFixed(2)}</td>
        <td class="exp-category">${expense.category}</td>
        <td class="exp-date">${
          expense.date
        }<button class="btn btn-danger float-right delete">X</button></td>
        `;
...
class DatePicker {
  static setTodayDateToInputDate() {
    ...
    document.querySelector(
      "#expense-date"
    ).value = `${day}-${month}-${today.getUTCFullYear()}`;
  }
